...
def add_transactions(data: Sequence[Transaction | TransactionImportView]):
    ...
    for transaction in data:
        try:
            transaction_repository.add_transaction(data=transaction)
    ...
    if number_skipped_transactions > 0:
        logger.debug(
            f"Skipped {number_skipped_transactions} "
...
def add_account_history_entries(data: list[tuple[int, float, str, str]]):
    ...
    logger.debug(
        "Account history entries successfully inserted to databaseor skipped."
    )
...
def update_account_balances(latest: dict[str, tuple[str, float, int]]) -> None:
    ...
    for account_number, (
        ...
        try:
            ...
            logger.debug(
                "Account balances successfully updated in the database."
            )
