...
def attach_existing_transaction_ids(
    transaction_data: list[Transaction], db_path: Path = config.Database.PATH
) -> list[Transaction]:
    # TODO: add docs
...
def build_transaction_view(transaction: Transaction) -> TransactionView:
    ...
    counterparty_data = get_counterparty_by_id(transaction.counterparty_id)
    assert counterparty_data is not None

    category_data = get_category_by_id(transaction.category_id)
